...
public class UpdatePlanUseCase {
    ...
    @Transactional
    public PlanView execute(UpdatePlanCommand command) {
        var plan = repository.findById(command.id())
                .orElseThrow(() -> new PlanNotFoundException(command.id()));
        plan.updateMaxOrganizations(command.maxOrganizations());
